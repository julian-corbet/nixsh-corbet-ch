...
# against. A nixpkgs copy on PATH shadows it, leaving two versions of the shell in play -- the
# login one and the interactive one. So `arch` here is what pacman installs, and the home-manager
# backend deliberately does NOT let home-manager install its own.
{ ... }:
{
  fish = {
    arch = "fish";
